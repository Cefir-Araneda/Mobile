...
export class ApiService {
  ...
  createPostL(post: any): Observable<any> {
    return this.http.post(this.apiURL + "/login", post, this.httpOptions).pipe(
      retry(3)
    )
  }
  ...
  getPostL(ID: any): Observable<any> {
    return this.http.get(this.apiURL + "/login/" + ID).pipe(
      retry(3)
    )
  }
  ...
  updatePostL(ID: any, post: any): Observable<any> {
    return this.http.put(this.apiURL + "/login/" + ID, post, this.httpOptions).pipe(
      retry(3)
    )
  }
  ...
  deletePostL(ID: any): Observable<any> {
    return this.http.delete(this.apiURL + "/login/" + ID).pipe(
      retry(3)
    )
  }
